...
function App() {
  ...
  const MarvelHash = CryptoJS.MD5(
    `1${MarvelPrivateKey}${MarvelPublicKey}`
  ).toString();
  const getTypeName = (i) => {
    switch (i) {
      case 0:
  ...
  const callApi = (value) => {
    const characters = axios.get(
      `https://gateway.marvel.com/v1/public/characters?nameStartsWith=${value}&apikey=${MarvelPublicKey}&ts=1&hash=${MarvelHash}&limit=3`
    );
    const series = axios.get(
      `https://gateway.marvel.com/v1/public/series?titleStartsWith=${value}&apikey=${MarvelPublicKey}&ts=1&hash=${MarvelHash}&limit=3`
    );
    const comics = axios.get(
      `https://gateway.marvel.com/v1/public/comics?titleStartsWith=${value}&apikey=${MarvelPublicKey}&ts=1&hash=${MarvelHash}&limit=3`
    );
          ...
          <div>
            ...
            <datalist id="languageList">
              {resultsData &&
                ...
                resultsData.map((r) => (
                  <option key={r.id} value={`#${r.id} ${getTypeName(r.type)}`}>
                    {r.type === 0 ? r.name : r.title}
                  </option>
                ))}
            </datalist>
          </div>
                  ...
                  i < 5 && (
                    <div>
                      <a
                        ...
                        onClick={() => loadPrev(p)}
                      >
                        {p.type === 0 ? p.data.name : p.data.title} (
                        {getTypeName(p.type)})
                      </a>
                      <br />
                      <br />
